...
export const Route = createFileRoute("/_app/students/$studentId")({
  ...
  head: () => ({
    meta: [
      { title: "Student Profile — SchoolSphere" },
    ],
  }),
});
...
function StudentProfilePage() {
  ...
  // Sample fee data
  const fees = [
    { term: "Term 1", amount: "₹15,000", paid: "₹15,000", status: "Paid", date: "Jun 2025" },
    { term: "Term 2", amount: "₹15,000", paid: "₹15,000", status: "Paid", date: "Oct 2025" },
    { term: "Term 3", amount: "₹15,000", paid: "₹10,000", status: "Partial", date: "Feb 2026" },
  ];

  // Sample library data
  const libraryBooks = [
    { title: "Physics Vol. 2", author: "H.C. Verma", issued: "Apr 05, 2026", due: "Apr 19, 2026", status: "Active" },
    { title: "Mathematics NCERT", author: "NCERT", issued: "Mar 20, 2026", due: "Apr 03, 2026", status: "Returned" },
  ];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                  ...
                  {attendance.map((a) => (
                    <tr key={a.id} className="border-b border-border last:border-0">
                      ...
                      <td className="px-3 py-2">
                        <StatusBadge status={a.status === "present" ? "Present" : "Absent"} variant={a.status === "present" ? "success" : "destructive"} />
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {/* Grades */}
        <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
          <h3 className="mb-4 text-lg font-semibold text-card-foreground">Exam Grades (Last Exam)</h3>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="px-3 py-2 text-left text-xs font-semibold text-muted-foreground">Subject</th>
                <th className="px-3 py-2 text-center text-xs font-semibold text-muted-foreground">Marks</th>
                <th className="px-3 py-2 text-center text-xs font-semibold text-muted-foreground">Grade</th>
              </tr>
            </thead>
            <tbody>
              {grades.map((g, i) => (
                <tr key={i} className="border-b border-border last:border-0">
                  ...
                  <td className="px-3 py-2 text-center">
                    <StatusBadge status={g.grade} variant={g.marks >= 85 ? "success" : g.marks >= 70 ? "info" : "warning"} />
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="px-3 py-2 text-left text-xs font-semibold text-muted-foreground">Term</th>
            ...
            <tbody>
              {fees.map((f, i) => (
                <tr key={i} className="border-b border-border last:border-0">
                  <td className="px-3 py-2 font-medium">{f.term}</td>
                  <td className="px-3 py-2">{f.amount}</td>
                  <td className="px-3 py-2">{f.paid}</td>
                  <td className="px-3 py-2">
                    <StatusBadge status={f.status} variant={f.status === "Paid" ? "success" : "warning"} />
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            <tbody>
              {libraryBooks.map((b, i) => (
                <tr key={i} className="border-b border-border last:border-0">
                  ...
                  <td className="px-3 py-2">
                    <StatusBadge status={b.status} variant={b.status === "Active" ? "info" : "success"} />
                  </td>
                </tr>
              ))}
            </tbody>
